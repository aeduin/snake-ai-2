...
class CnnAi(nn.Module):
    def __init__(self, world: World):
        ...
        self.conv1 = nn.Conv2d(simulation.num_channels, 8, (3, 3)).to(device)
        size = (size - 2)
        self.conv2 = nn.Conv2d(8, 8, (3, 3)).to(device)
        size = (size - 2)
        self.conv3 = nn.Conv2d(8, 8, (3, 3)).to(device)
        size = (size - 2)
        self.dense = nn.Linear(int(size[0] * size[1] * 8), 4).to(device)
